...
edss_calculation <- function(score_pyramidal, score_cerebellar, score_brain_stem, score_sensory,
  ...
  if (edss_method == "neurostatus_16") {
    ...
    } else if (score_ambulation == 1 && all(subsystems <= 0)) {
      return(2)
      ## Ambulation score = 1, no FS higher than 0, EDSS = 2
    } else if (count_1 == 1 && all(subsystems <= 1) && score_ambulation == 1) {
      return(2)
      ## Exactly one FS score = 1, others ≤ 1, Ambulation score = 1
    } else if (count_1 >= 2 && all(subsystems <= 1) && score_ambulation == 1) {
      return(2)
      ## Two or more FS scores = 1, all FS scores ≤ 1, Ambulation score = 1
  ...
  else if (edss_method == "neurostatus_12") {
    ## EDSS calculation logic
    if (count_0 == 7 && score_ambulation == 0) {
      return(0)
      ## All FS scores = 0 and Ambulation score = 0
